...
def fmerge_at_z(model, zbin_low, zbin_high, sigmaZ, cosmic=False, cbc_type=None, zmerge_min=0, zmerge_max=0.1):
    """
...
def local_rate(model, zmin, zmax, cosmic=False, cbc_type=None, zmerge_min=0, zmerge_max=0.1, sigmaZ=0.5, N_zbins=100):
    """
    ...
    for zbin_low, zbin_high in tqdm(zip(zbins[::-1][1:], zbins[::-1][:-1]), total=len(zbins)-1):
        ...
        floc = fmerge_at_z(model, zbin_low, zbin_high, sigmaZ, cosmic, cbc_type, zmerge_min=zmerge_min, zmerge_max=zmerge_max)
